...
const stopwords_json_1 = __importDefault(require("./stopwords.json"));
...
function getWords(text, language = english) {
    let regex = new RegExp(stopwords_json_1.default[language].wordregex[0], stopwords_json_1.default[language].wordregex[1]);
    let words = [];
    let indices = [];
...
function normalizeAndRemoveStopWords(words, indicesList, language = english) {
    let stopwords = stopwords_json_1.default[language].stopwords;
    let spaces = RegExp(stopwords_json_1.default[language].spaceregex[0], stopwords_json_1.default[language].spaceregex[1]); // All non-allowed characters
    let newWords = [];
    let newIndicesList = [];
...
function singleSearchScrape(query) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let response = yield get(url.href, 60000);
        let anchorTags = response.match(/<a[\s]+([^>]+)>/gi) || [];
        let urls = [];
        for (let tag of anchorTags) {
            let link = ((_a = tag.match(/".*?"/)) === null || _a === void 0 ? void 0 : _a[0]) || "";
...
function match({ text = "", language = english, shingleSize = 2, apikey = process.env.G_API_KEY || "", engineid = process.env.G_ENGINE_ID || "", maximumGap = 3, minimumClusterSize = 5, verbose = false } = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let query of searchQueries) {
            ...
            let [comparedTexts, comparedTitles] = (yield downloadWebsites(comparedUrls, true, verbose).catch(console.log)) || [[], []];
